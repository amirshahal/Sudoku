...
class Sudoku:
    ...
    def do_trial(self):
        # print(f"Do trial. self.trial is {self.trial}")
        ...
        cell = self.cells[row][col]
        if cell.value is None:
            if val in cell.available_values:
                self.solved_step += 1
                self.cells[row][col].solved(self.solved_step, val, True)
                self.update_all_available_values(False)
            else:
                print(f"Cant use value {val} at cell({row},{col}) which is not avail in this cell")
        else:
    ...
    def update_all_available_values(self, update_derived):
        while True:
            avail_changed = False
            ...
            for row in range(9):
                for col in range(9):
                    cell = self.cells[row][col]
                    if cell.value is None:
                        avail_changed = avail_changed or self.update_available_values(cell)
            ...
            if update_derived:
                for row in range(9):
                    for col in range(9):
                        cell = self.cells[row][col]
                        if cell.value is None:
                            derive_changed = derive_changed or self.remove_derived_values(cell)
    ...
    def update_available_values(self, cell):
        ...
        changed = changed or self.remove_non_available_values(cell, 'col')
        changed = changed or self.remove_non_available_values(cell, 'square')
        return changed
    ...
    def solve_next_cell(self, second_run = False):
                ...
                for col in range(9):
                    ...
                    if cell.value is None:
                        ...
                        if len(cell.available_values) == 1:
                            ...
                            cell.solved(self.solved_step, list(cell.available_values)[0])
                            self.update_all_available_values(False)
                            if self.verbose > 0:
        ...
        if not second_run and not solved_cell:
            ...
            self.update_all_available_values(True)
                    ...
                    for col in range(9):
                        if self.cells[row][col].value == value:
                            ...
                            break
                                ...
                                value in self.cells[row][col].available_values:
